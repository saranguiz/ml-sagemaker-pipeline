...
import smdebug.pytorch as smd # dependencies for Debugging andd Profiling
...
def train(model, epochs, train_loader, validation_loader, criterion, optimizer, model_dir, hook):
    best_acc = 0.0
...
def main(args):
    tic = time.time()
    logger.info(f'Learning rate: {args.lr}') # Remove
    logger.info(f'Batch size: {args.train_batch_size}') # Remove
    train_loader, test_loader, validation_loader = create_data_loaders(args.train_data_dir, args.train_batch_size)
    model = net() # Initialize a model by calling the net function
    
    hook = smd.Hook.create_from_json_file() # For debugging and profiling
    hook.register_hook(model)

    criterion = nn.CrossEntropyLoss()
    optimizer = optim.Adam(model.fc.parameters(), lr=args.lr)
    logger.info(f'Starting Model Training')
    model=train(model, args.epochs, train_loader, validation_loader, criterion, optimizer, args.model_dir, hook)
...
if __name__=='__main__':
    ...
    parser.add_argument(
        "--train-batch-size", type=int, default=5, metavar="N", help="input batch size for training, validation and testing (default: 5)",
    )
    parser.add_argument(
